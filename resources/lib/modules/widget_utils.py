...
def widget_monitor(list_id):
	...
	while not monitor.abortRequested():
		...
		if last_path == cpath_path or xbmc.getCondVisibility('System.HasActiveModalDialog'): continue
		switch_widget = True
		countdown = delay
		while not monitor.abortRequested() and countdown >= 0 and switch_widget:
			monitor.waitForAbort(0.25)
			countdown -= 0.25
			if list_id != str(window.getFocusId()): switch_widget = False
			if last_path == cpath_path: switch_widget = False
